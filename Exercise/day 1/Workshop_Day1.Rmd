...
self-containing, and while the lecture slides that go with it add a bit of onfo, you should be able
...
It looks at co-occurrence patterns in different plants along an elevational gradient in the Swiss Alps. The original dataset can be found at https://doi.org/10.5061/dryad.8mv11, which also contains links to a fantastic paper by Manuela D'Amen, 
Heidi Mod, Nicholas Gotelli and Antoine Guisan which uses the data, entitled "Disentangling biotic 
interactions, environmental filters, and dispersal limitation as drivers of species co-occurrence". The elevational
data we use here comes from SwissTopo.
